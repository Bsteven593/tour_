...
function GuideRol() {
  ...
  const validateForm = () => {
    ...
    if (!formData.username) {    // Validación del campo username
      AlertRegister.showError('Atento', "El campo 'Nombre de Usuario' es obligatorio.");
      isValid = false;
    }
    if (!formData.dni) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        const newUser = await createGuide(formData);
        console.log('Nuevo usuario creado:', newUser);
        Swal.fire({
          title: 'Éxito',
          text: 'Guía creado con éxito',
          icon: 'success',
    ...
    <div>
      <div className="container p-10">
        <form className="formulario__register" onSubmit={handleSubmit}>
          <h2 className="text-center">Registra un Guía</h2>
          <div className="row">
            <div className="col-md-6">
              ...
              <div className="mb-3">
                <label htmlFor="username" className="form-label"><b>*</b>Nombre de Usuario:</label>    {/* Añadido campo username */}
